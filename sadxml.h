        ...
        if(buf[i] == '<'){
            ...
            if(buf[i+1] == '/'){
                ...
                if(strcmp(curr_node->tag, lex)){
                    fprintf(stderr, "Mismatched Start and End Tags (%s)vs(%s)\n", curr_node->tag, lex);
                    return FALSE;
                }
            ...
            while(buf[i] != '>'){
                ...
                // In inner text just ignore spaces
                if(lex[lexi-1] == ' ' ){
                    lexi--;
                    continue;
                }
                ...
                if(buf[i] == '"'){
                    if(!curr_attr.key){
                        fprintf(stderr, "No key Value already, there cannot be value pair\n");
                        return FALSE;
                    }
